...
Nginx는 고성능 이벤트 기반 구조를 가진 **웹 서버**이자, **리버스 프록시** 및 **L7 로드밸런서** 역할을 수행합니다. 특히 로드밸런싱 측면에서는 L7 계층에서 동작하기 때문에, 단순 IP/포트 분산을 넘어 **HTTP 헤더, URL 경로, 쿠키** 등을 분석하여 정교한 트래픽 분산이 가능하다는 장점이 있습니다.
